...
void minus(double** first, double** second, double** whatWriteTo, int M, double N) {    
    int i,j;
...
double scalarProduct(double** first, double** second, int M, int N, double h1, double h2) {
    double sum = 0.0;
...
void multiplyByNum(double** items, double num, double** whatWriteTo, int M, int N) {
    #pragma omp parallel for collapse(2)
    for (size_t i = 0; i <= M; ++i) {
            for (size_t j = 0; j <= N; ++j) {
...
int main(int argc, char** argv) {
    ...
    double time_taken = t; //((double)t)/CLOCKS_PER_SEC;
    double max = 0.0;
    for (size_t i = 0; i <= M; ++i) {
        for (size_t j = 0; j <= N; ++j) { 
